...
export const useSoundEffect = (letter) => {
  ...
  if (db && db[letter]) {
    ...
    tracks.push(`${window.location.origin}/audio/${path}${letter}.mp3`);
    tracks.push(`${window.location.origin}/audio/${path}as_in.mp3`);
    tracks.push(`${window.location.origin}/audio/${path}${letter}/${choice}`);
